...
TEST_CASE("shrink() halves capacity when sz <= cap/4") {
    ...
    // We want to land at exactly cap==16 for a predictable shrink test.
    // Your growth policy doubles, so cap should be 16 here.
    REQUIRE(v.capacity() == 16);

    // Pop until size <= cap/4 => cap/4 = 4, so shrink triggers when sz <= 4
    for (int i = 0; i < 12; ++i) v.pop_back(); // size 4
    REQUIRE(v.size() == 4);

    // shrink() is called inside pop_back, so by now cap should have halved to 8
    REQUIRE(v.capacity() == 8);

    // Pop down to size 2 -> cap/4 = 2, so shrink triggers again
    v.pop_back(); // size 3 (no shrink yet because 3 > 8/4=2)
    REQUIRE(v.capacity() == 8);

    v.pop_back(); // size 2 (now <= 2, shrink -> cap 4)
    REQUIRE(v.size() == 2);
...
TEST_CASE("shrink_to_fit sets capacity to max(1, sz) and preserves elements") {
    ...
    // remove some to make sz smaller than cap
...
TEST_CASE("Works with non-int type (basic string test)") {
    Vector<std::string> v;
    v.push_back("a");
    v.push_back("b");
    v.insert(1, "X"); // [a X b]
